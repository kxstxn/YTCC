...
async def cut(url):
    ...
    if os.path.isdir('clipped'): #Проверяем существует ли дирректория + создаём если ее нет
        ...
        os.mkdir('clipped')
    while end <= video_len: #Нарезаем оришинальное видео на клипы
        clip = VideoFileClip('downloaded/video.mp4').subclipped(start, end)
        clip.write_videofile(f'clipped/clip{str(clip_num)}.mp4') #Сохраняем каждый клип в заданную лирректорию
        start += 60
        end += 60
        clip_num += 1
